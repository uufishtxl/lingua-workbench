  ...
  <div class="dark-editor-base dark-textarea h-[210px] p-2 rounded flex flex-col gap-1 overflow-x-hidden">
    ...
    <div class="flex items-center justify-between gap-1.5">
      ...
      <div class="flex items-center gap-1.5 flex-1">
        <!-- Mode Toggle Button -->
        <el-button text circle class="dark-bg-controls"
        ...
        <div class="relative flex-1">
          <el-input ref="noteInput" v-model="editableHighlight.note" type="textarea" :rows="1"
            :placeholder="aiAnalysis.editorMode.value === 'note' ? '添加笔记...' : '编辑发音...'" class="w-full note-input"
            @keydown.enter.prevent="handleSave_Input" />
          <!-- Save Note Button (inside input) -->
          <button class="save-note-btn-inline" @click="handleSave_Input" :disabled="!editableHighlight.note?.trim()"
      ...
      <div class="flex items-center gap-0.5 shrink-0">
        <el-button text circle class="dark-bg-controls"
          ...
          :title="aiAnalysis.isFastSpeed.value ? '极速语流' : '日常口语'" dark-bg-controls>
          <span class="text-md">{{ aiAnalysis.isFastSpeed.value ? '🐇' : '🐢' }}</span>
        </el-button>
        <el-button text size="small" circle class="ai-btn dark-bg-controls"
          :class="{ 'is-loading': aiAnalysis.aiStatus.value === 'loading' }" @click="handleAiClick"
          title="点击获取 AI 语音图谱">
    ...
    <div v-if="aiAnalysis.editorMode.value === 'note' && aiAnalysis.analysisResult.value?.phonetic_tags?.length"
      class="flex flex-wrap gap-1 text-[10px]">
      <div v-for="(tag, idx) in aiAnalysis.analysisResult.value.phonetic_tags" :key="idx" class="segment-note"
        :class="aiAnalysis.getTypeClass(tag)" @click="handleTagClick(tag)">
    ...
    <div v-if="aiAnalysis.editorMode.value === 'sound' && aiAnalysis.analysisResult.value?.script_segments?.length"
      class="flex flex-wrap gap-1 text-[10px]">
      <div v-for="(seg, idx) in aiAnalysis.analysisResult.value.script_segments" :key="idx"
        class="segment-note segment-sound-item" :class="{
    ...
    <!-- Dictionary Section -->
    <DictionarySection :dictionary-result="dictionary.dictionaryResult.value" :dict-status="dictionary.dictStatus.value"
      :example-status="dictionary.exampleStatus.value" :show-english-example="dictionary.showEnglishExample.value"
      :is-editing-definition="dictionary.isEditingDefinition.value" @dict-click="handleDictClick"
      @toggle-language="dictionary.toggleExampleLanguage" @refresh-example="dictionary.handleRefreshExample"
      @definition-click="handleDefinitionClick" />

    <!-- Actions -->
    <div class="flex justify-between mt-auto">
      <div>
        <el-button text circle type="primary" class="dark-bg-controls" @click="emit('play-original')">
          <i-tabler-player-pause-filled v-if="props.isPlaying" class="text-sm" />
...
/* Type colors */
.type-reduction .type-badge {
  background: #fbbf24;
  color: #000;
}

.type-linking .type-badge {
  background: #60a5fa;
  color: #000;
}

.type-assimilation .type-badge {
  background: #34d399;
  color: #000;
}

.type-elision .type-badge {
  background: #f472b6;
  color: #000;
}

.type-flap .type-badge {
  background: #a78bfa;
  color: #000;
}

.type-glottal .type-badge {
  background: #fb923c;
  color: #000;
}
